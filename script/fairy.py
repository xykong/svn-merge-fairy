...
class MergeFairy:
    ...
    mail on success of failure.  It also attempts to automatically merge changes
    into any dependendent branches and build those branches.
            ...
            for revision in revisions:
                ...
                for dependency in self.dependencyMap.getDependencies(branch):
                    ...
                    if failedMergedVersion == -1:  # if no blocking revisions
                        ...
                        printAndLog("Blocked on auto-merging version %s by earlier merge failure %s" % (revision.number, dependency.failedMergeVersion))
        ...
        if not dependency:  # we couldn't find any branch needing a merge for the given revision
            ...
            if dependency.dependentBranch != branch:
                ...
                self.sendMail(mergeFairyEmail, [recipient], resolveWrongBranchMessage[0], resolveWrongBranchMessage[1] % \
                    {'resolvedVersion': resolvedVersion, 'checkinBranch': branch, 'dependentBranch': dependency.dependentBranch})
            else:
                ...
                for blockedRevision in dependency.baseBranch.getPendingRevisions(failedMergeVersion + 1):
                    ...
                    if not mergeOK:       # bail out if the merge failed
                        break
                self.sendMail(mergeFairyEmail, [recipient], resolveSucceededMessage[0], resolveSucceededMessage[1] % {'resolvedVersion': resolvedVersion})
                self.sendMail(happyMergeFairyEmail, [groupRecipients], resolveSucceededMessage[0], resolveSucceededMessage[1] % {'resolvedVersion': resolvedVersion})
    ...
    def mergeRevision(self, dependency, revision):
        ...
        try:
            ...
            args = {'number': revision.number, 'branch1': revision.branch, 'branch2': dependentBranch, 'dev': revision.dev, 'startVersion': self.getStartMergeVersion(dependency, revision.number)}
		...
		printAndLog(mergeConflictMessage[1] % args)
                ...
                if groupRecipients:
                    self.sendMail(madMergeFairyEmail, [groupRecipients], mergeConflictMessage[0], mergeConflictMessage[1] % args)
            ...
            # otherwise try building
                ...
                if buildSucceeded:
                    ...
                    dependency.failedMergeDev = revision.dev
                    dependentBranch.revertChanges()
                    self.sendMail(madMergeFairyEmail, [recipient], mergeBuildFailedMessage[0], mergeBuildFailedMessage[1] % args)
		    if groupRecipients:
			self.sendMail(madMergeFairyEmail, [groupRecipients], mergeBuildFailedMessage[0], mergeBuildFailedMessage[1] % args)
                return (False, "failed")
            else:
...
class SvnBranch:
	...
	printAndLog("Loaded branch %s lastBuildVersion=%d lastSyncVersion=%d" % (self.url, self.lastBuildVersion, self.lastSyncVersion))
        return self
    ...
    def update(self, revision):
        ...
        fUpdated = False
        for line in self.popen('svn --username "%s"  --password "%s" -r %d update %s' % (svnUserName, svnPassword, revision.number, self.path)):
    ...
    def mergeChanges(self, mergeCmd):
        ...
        for line in self.popen(mergeCmd):
            printAndLog(line)
            fChanged = True
            if re.match("^C ", line) or re.match("^svn:", line) or re.match("Skipped missing target:", line) or re.match("has different repository root", line):
               fConflicted = True
    ...
    def commitChanges(self, commitMessage):
        ...
        if not options.fake_commit:
            ...
            commitFile.flush()
            cmd = 'svn commit --username "%s"  --password "%s" -F "%s" %s' % (svnUserName, svnPassword, commitFile.name, self.path)
            printAndLog(cmd)
            for line in self.popen(cmd):
                printAndLog(line)
...
class Dependency:
	...
	printAndLog("Loaded dependency %s=>%s lastGoodMerge=%d failedMergeVersion=%d failedMergeDev=%s" % (self.baseBranch, self.dependentBranch, self.lastGoodMerge, self.failedMergeVersion, self.failedMergeDev))
        return self
...
def startstop(stdout='/var/log/merge-fairy.log', stderr='/var/log/merge-fairy.err', stdin='/dev/null', pidfile=pidFileDir + '/fairy.pid', startmsg = 'started with pid %s', action='start' ):
    if action:
        try:
            pf  = file(pidfile,'r')
...
if __name__ == "__main__":
    parser = OptionParser()
    parser.add_option("-C", action="store", dest="config_file_name", default="fairy.xml", help="configuration file path")
    parser.add_option("-e", "--forceemail", action="store", dest="force_email", help="(DEBUG ONLY) force all emails to go to the given address")
    parser.add_option("-v", "--verbose", action="store_true", dest="verbose", help="display additional debugging information.")
    parser.add_option("-i", "--interactive", action="store_true", dest="interactive", help="(DEBUG ONLY) wait for a keystroke between iterations rather than sleeping")
    parser.add_option("-f", "--fakecommit", action="store_true", dest="fake_commit", help="(DEBUG ONLY) don't actually commit any changes")
    parser.add_option("-s", "--start", action="store_true", dest="start_daemon", help="start the fairy as a daemon")
    parser.add_option("-r", "--restart", action="store_true", dest="restart_daemon", help="restart the fairy as a daemon")
